...
class SBAdyenCC extends WC_Payment_Gateway_CC {
    ...
    public function payment_fields() {
        ...
        $date_time = date( 'j F Y h:i:s', strtotime( 'now' ) );
        ...
        // Set Adyen test or live environment
        ...
        // init checkout and log status
        try {
            $checkout = new \Adyen\Service\Checkout( $client );
            file_put_contents( SB_ADYEN_PATH . 'logs/cc/success/checkout_init.log', $date_time . ': ' . print_r( $checkout, true ), FILE_APPEND );
        } catch ( Exception $ex ) {
            file_put_contents( SB_ADYEN_PATH . 'logs/cc/error/checkout_init.log', $date_time . ': ' . print $ex->getMessage(),  FILE_APPEND );
        }
        ...
        // setup transaction payload array
        $payload = [
                'amount'          => array(
                        'currency' => $currency,
                        'value'    => $value
        ...
        // start Adyen session and log data 
        try {
            $session = $checkout->sessions( $payload );
            file_put_contents( SB_ADYEN_PATH . 'logs/cc/success/session_init.log', $date_time . ': ' . print_r( $session, true ), FILE_APPEND );
        } catch ( Exception $ex ) {
            file_put_contents( SB_ADYEN_PATH . 'logs/cc/error/session_init.log', $date_time . ': ' . print $ex->getMessage(),  FILE_APPEND );
        }
        ...
        <script type="text/javascript">
            ...
            var client_key = '<?php echo ADYEN_CLIENT_KEY; ?>';
            ...
            cc_configuration = {
                ...
                session: {
                    ...
                    sessionData: '<?php echo $session[ "sessionData" ] ?>'
                },
    ...
    public function process_payment($order_id) {
        ...
        if ( isset( $_REQUEST[ 'stateData' ] ) && !empty( $_REQUEST[ 'stateData' ] ) && !empty( $_REQUEST[ 'stateData' ][ 'type' ] ) ) {
            ...
            // setup missing address data if present
            ...
            try {
                ...
                // log
                file_put_contents( SB_ADYEN_PATH . 'logs/cc/success/pmt_request.log', date( 'j F Y h:i:s', strtotime( $_SERVER[ 'REQUEST_TIME' ] ) ) . ': ' . print_r( $payment_request, true ), FILE_APPEND );
