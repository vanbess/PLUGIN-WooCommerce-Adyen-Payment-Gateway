...
/**
 * Renders FINNISH ONLINE BANKING payment option
 *
 * @extends WC_Payment_Gateway
 * @version 1.0.0
 * @author Werner C. Bessinger @ Silverback Dev
 */
class SBAdyenFOB extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        /* method id */
        $this->id = 'sb-adyen-fob';

        /* method title */
        $this->method_title = esc_attr__('SB Adyen FINNISH ONLINE BANKING', 'sb-adyen-fob');

        /* method description */
        $this->method_description = esc_attr__('Adyen Finnish Online Banking payments.', 'sb-adyen-fob');

        /* gateway icon */
        $this->icon = plugin_dir_url(__FILE__) . 'images/ebanking_FI.svg';
    ...
    public function settings_fields()
    {
        ...
        $this->form_fields = [
            /* enable/disable */
            'enabled' => [
                'title'   => esc_attr__('Enable / Disable', 'sb-adyen-fob'),
                'label'   => esc_attr__('Enable this payment gateway', 'sb-adyen-fob'),
                'type'    => 'checkbox',
                'default' => 'no',
            ],
            'title'   => [
                'title'       => __('Title', 'sb-adyen-fob'),
                'type'        => 'text',
                'description' => __('Add your custom payment method title here if required.', 'sb-adyen-fob'),
                'default'     => __('Finnish Online Banking', 'sb-adyen-fob'),
            ]
        ];
    }
    ...
    public function process_payment($order_id)
    {
        ...
        $order->add_order_note(__('Awaiting FINNISH ONLINE BANKING payment', 'woocommerce'));
        ...
        try {
            ...
            file_put_contents(SB_ADYEN_PATH . 'error-logs/fob/error.log', $_SERVER['REQUEST_TIME'] . ': ' . $ex->getMessage() . PHP_EOL, FILE_APPEND);
        }
