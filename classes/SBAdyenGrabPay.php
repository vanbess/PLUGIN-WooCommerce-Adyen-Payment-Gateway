...
/**
 * Renders GRABPAY payment option
 *
 * @extends WC_Payment_Gateway
 * @version 1.0.0
 * @author Werner C. Bessinger @ Silverback Dev
 */
class SBAdyenGrabPay extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        /* method id */
        $this->id = 'sb-adyen-gpay';

        /* method title */
        $this->method_title = esc_attr__('SB Adyen GRABPAY', 'sb-adyen-gpay');

        /* method description */
        $this->method_description = esc_attr__('Adyen GrabPay payments.', 'sb-adyen-gpay');

        /* gateway icon */
        $this->icon = plugin_dir_url(__FILE__) . 'images/grabpay-logo.png';
    ...
    public function settings_fields()
    {
        ...
        $this->form_fields = [
            /* enable/disable */
            'enabled' => [
                'title'   => esc_attr__('Enable / Disable', 'sb-adyen-gpay'),
                'label'   => esc_attr__('Enable this payment gateway', 'sb-adyen-gpay'),
                'type'    => 'checkbox',
                'default' => 'no',
            ],
            'title'   => [
                'title'       => __('Title', 'sb-adyen-gpay'),
                'type'        => 'text',
                'description' => __('Add your custom payment method title here if required.', 'sb-adyen-gpay'),
                'default'     => __('GrabPay', 'sb-adyen-gpay'),
            ]
        ];
    }
    ...
    public function process_payment($order_id)
    {
        ...
        $order->add_order_note(__('Awaiting GrabPay payment', 'woocommerce'));
        ...
        try {
            ...
            file_put_contents(SB_ADYEN_PATH . 'error-logs/grabpay/error.log', $_SERVER['REQUEST_TIME'] . ': ' . $ex->getMessage() . PHP_EOL, FILE_APPEND);
        }
        ...
        try {
            ...
            $order->add_order_note(__('Payment error: ' . $ex->getMessage(), 'woocommerce'));
        }
