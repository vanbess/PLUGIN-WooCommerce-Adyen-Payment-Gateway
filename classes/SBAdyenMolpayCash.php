...
/**
 * Renders MOLPAY CASH (7-ELEVEN MALAYSIA) payment option
 *
 * @extends WC_Payment_Gateway
 * @version 1.0.0
 * @author Werner C. Bessinger @ Silverback Dev
 */
class SBAdyenMolpayCash extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        /* method id */
        $this->id = 'sb-adyen-mcash';

        /* method title */
        $this->method_title = esc_attr__( 'SB Adyen 7-ELEVEN MALAYSIA', 'sb-adyen-mcash' );

        /* method description */
        $this->method_description = esc_attr__( 'Adyen 7-Eleven Malaysia  payments.', 'sb-adyen-mcash' );

        /* gateway icon */
        $this->icon = plugin_dir_url( __FILE__ ) . 'images/7-eleven-logo.png';
    ...
    public function settings_fields() {
        ...
        $this->form_fields = [
                /* enable/disable */
                'enabled' => [
                        'title'   => esc_attr__( 'Enable / Disable', 'sb-adyen-mcash' ),
                        'label'   => esc_attr__( 'Enable this payment gateway', 'sb-adyen-mcash' ),
                        'type'    => 'checkbox',
                        'default' => 'no',
                ],
                'title'   => [
                        'title'       => __( 'Title', 'sb-adyen-mcash' ),
                        'type'        => 'text',
                        'description' => __( 'Add your custom payment method title here if required.', 'sb-adyen-mcash' ),
                        'default'     => __( '7-Eleven Malaysia', 'sb-adyen-mcash' ),
                ]
        ];
    }
    ...
    public function process_payment($order_id) {
        ...
        $order->add_order_note( __( 'Awaiting 7-Eleven (Malaysia) payment', 'woocommerce' ) );
        ...
        try {
            ...
            file_put_contents( SB_ADYEN_PATH . 'error-logs/molpay-cash/error.log', $_SERVER[ 'REQUEST_TIME' ] . ': ' . $ex->getMessage() . PHP_EOL, FILE_APPEND );
        }
