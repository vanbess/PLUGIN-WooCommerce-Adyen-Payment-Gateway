...
/**
 * Renders QIWI payment option
 *
 * @extends WC_Payment_Gateway
 * @version 1.0.0
 * @author Werner C. Bessinger @ Silverback Dev
 */
class SBAdyenQiwi extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        /* method id */
        $this->id = 'sb-adyen-qiwi';

        /* method title */
        $this->method_title = esc_attr__('SB Adyen QIWI WALLET', 'sb-adyen-qiwi');

        /* method description */
        $this->method_description = esc_attr__('Adyen Qiwi Wallet payments.', 'sb-adyen-qiwi');

        /* gateway icon */
        $this->icon = plugin_dir_url(__FILE__) . 'images/qiwi-logo.png';
    ...
    public function settings_fields()
    {
        ...
        $this->form_fields = [
            /* enable/disable */
            'enabled' => [
                'title'   => esc_attr__('Enable / Disable', 'sb-adyen-qiwi'),
                'label'   => esc_attr__('Enable this payment gateway', 'sb-adyen-qiwi'),
                'type'    => 'checkbox',
                'default' => 'no',
            ],
            'title'   => [
                'title'       => __('Title', 'sb-adyen-qiwi'),
                'type'        => 'text',
                'description' => __('Add your custom payment method title here if required.', 'sb-adyen-qiwi'),
                'default'     => __('Qiwi', 'sb-adyen-qiwi'),
            ]
        ];
    }
    ...
    public function process_payment($order_id)
    {
        ...
        $order->add_order_note(__('Awaiting Qiwi Wallet payment', 'woocommerce'));
        ...
        if (ADYEN_GATEWAY_MODE == 'test') :
            $curl_url       = 'https://checkout-test.adyen.com/v51/payments';
            $order_currency = 'RUB';
        else :
            $curl_url       = "https://$url_prefix-checkout-live.adyenpayments.com/checkout/v51/payments";
        ...
        try {
            ...
            try {
                ...
                $order->add_order_note('cURL JSON response: ' . $response);
            ...
            $order->add_order_note(__('cURL failed to initialize with: ' . $ex->getMessage(), 'woocommerce'));
        }
    }
