...
/**
 * Renders YANDEX MONEY payment option
 *
 * @extends WC_Payment_Gateway
 * @version 1.0.0
 * @author Werner C. Bessinger @ Silverback Dev
 */
class SBAdyenYandexMoney extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        /* method id */
        $this->id = 'sb-adyen-yamon';

        /* method title */
        $this->method_title = esc_attr__( 'SB Adyen YANDEX MONEY', 'sb-adyen-yamon' );

        /* method description */
        $this->method_description = esc_attr__( 'Adyen Yandex Money payments.', 'sb-adyen-yamon' );

        /* display frontend title */
        $this->title = $this->get_option( 'title' );

        /* gateway icon */
        $this->icon = plugin_dir_url( __FILE__ ) . 'images/yandex-money.png';
    ...
    public function settings_fields() {
        ...
        $this->form_fields = [
                /* enable/disable */
                'enabled' => [
                        'title'   => esc_attr__( 'Enable / Disable', 'sb-adyen-yamon' ),
                        'label'   => esc_attr__( 'Enable this payment gateway', 'sb-adyen-yamon' ),
                        'type'    => 'checkbox',
                        'default' => 'no',
                ],
                'title'   => [
                        'title'       => __( 'Title', 'sb-adyen-yamon' ),
                        'type'        => 'text',
                        'description' => __( 'Add your custom payment method title here if required.', 'sb-adyen-yamon' ),
                        'default'     => __( 'Yandex Money', 'sb-adyen-yamon' ),
                ]
        ];
    }
    ...
    public function process_payment($order_id) {
        ...
        $order->add_order_note( __( 'Awaiting Yandex Money payment', 'woocommerce' ) );
        ...
        try {
            ...
            file_put_contents( SB_ADYEN_PATH . 'error-logs/yandex-money/error.log', $_SERVER[ 'REQUEST_TIME' ] . ': ' . $ex->getMessage() . PHP_EOL, FILE_APPEND );
        }
