...
function sb_adyen_custom_ty_page()
{
    ...
    if (!empty($_GET['resultCode'])) :
        ...
        $adyen_payment_methods = [
            ...
            'sb-adyen-gpay'       => 'GPay',
            'sb-adyen-molpay-th'  => 'Molpay Ebanking Thailand',
            'sb-adyen-molpay-my'  => 'Molpay Ebanking Malaysia',
        ...
        // setup error message
        $err_msg = __("Unfortunately your payment via <b>$pmt_method_purrty</b> was unsuccessful. <b>ERROR CODE: $resultCode</b>. Please return to the <a href='$checkout_page_url'><u>checkout page</u></a> and try again, or use a different payment method.", 'woocommerce');
...
?>
                ...
                <script type="text/javascript" id="custom-adyen-transaction-error" data-err-msg="<?php echo $err_msg; ?>">
                    jQuery(function($) {

                        var err_msg = $('#custom-adyen-transaction-error').data('err-msg');
                        ...
                        $('#content > div > div.woocommerce').prepend('<span id="sb-adyen-order-failed">' + err_msg + '</span>');
                    });
                </script>
...
<?php
                ...
                // add order note
                $order_data->add_order_note(__("Payment failed/is pending with reason: <b>$resultCode</b>."));
